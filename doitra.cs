...
{
    public partial class doitra : XtraForm
    {
        ...
        private void doitra_Load(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string sql = "SELECT MaPhieuTra, TenNCC, TenNV, NgayTra, LyDoTra, TongTien, TrangThai FROM PhieuTraHang p JOIN NhaCungCap n ON p.MaNCC = n.MaNCC JOIN NhanVien nv ON p.MaNV = nv.MaNV";
                SqlDataAdapter da = new SqlDataAdapter(sql, conn);
                dt = new DataTable();
                da.Fill(dt);
                gridControlphieudoitra.DataSource = dt;
            }


            this.gridViewphieutrahang.MouseUp += GridViewnphieutrahang_MouseUp;
            ...
            lblthoigian.Margin = new Padding(0, 19, 1070, 20);
            ...
            gridViewphieutrahang.OptionsSelection.MultiSelect = true;
            gridViewphieutrahang.OptionsSelection.MultiSelectMode =
                DevExpress.XtraGrid.Views.Grid.GridMultiSelectMode.CheckBoxRowSelect;

            // Hiển thị ô vuông ở header để chọn tất cả
            gridViewphieutrahang.OptionsSelection.ShowCheckBoxSelectorInColumnHeader = DevExpress.Utils.DefaultBoolean.True;

            // Ẩn cột indicator (số thứ tự dòng) để checkbox ra ngoài cùng
            gridViewphieutrahang.OptionsView.ShowIndicator = false;

            // --- Cấu hình giao diện ---
            gridViewphieutrahang.OptionsView.ShowGroupPanel = false;
            gridViewphieutrahang.OptionsView.EnableAppearanceEvenRow = true;
            gridViewphieutrahang.Appearance.HeaderPanel.Font = new Font("Arial", 10, FontStyle.Bold);
            gridViewphieutrahang.Appearance.Row.Font = new Font("Arial", 10);
            gridViewphieutrahang.RowHeight = 28;
            gridViewphieutrahang.OptionsView.ShowIndicator = false;
            //
            gridViewphieutrahang.OptionsBehavior.Editable = false;
            gridViewphieutrahang.OptionsBehavior.ReadOnly = true;
            gridViewphieutrahang.OptionsSelection.EnableAppearanceFocusedCell = false;

            gridViewphieutrahang.Columns["MaPhieuTra"].Caption = "Mã phiếu trả";
            gridViewphieutrahang.Columns["TenNCC"].Caption = "Tên nhà cung cấp";
            gridViewphieutrahang.Columns["TenNV"].Caption = "Tên nhân viên";
            gridViewphieutrahang.Columns["NgayTra"].Caption = "Ngày Trả";
            gridViewphieutrahang.Columns["LyDoTra"].Caption = "Lý do trả";
            gridViewphieutrahang.Columns["TongTien"].Caption = "Tổng tiền";
            gridViewphieutrahang.Columns["TongTien"].AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Near;
            gridViewphieutrahang.Columns["TrangThai"].Caption = "Trạng thái";
            gridViewphieutrahang.Columns["TrangThai"].AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Near;
            gridViewphieutrahang.Appearance.Row.Font = new Font("Arial", 10);
            gridViewphieutrahang.Appearance.HeaderPanel.Font = new Font("Arial", 11, FontStyle.Bold);
            //popuploctheo
        ...
        {
            gridViewphieutrahang.ActiveFilterString = "";  // Xóa điều kiện lọc
            gridViewphieutrahang.ClearColumnsFilter();     // Xóa filter ở từng cột (nếu có)
        ...
        private void btncaidat_Click(object sender, EventArgs e)
        {
            ...
            foreach (DevExpress.XtraGrid.Columns.GridColumn col in gridViewphieutrahang.Columns)
            {
        ...
        private void txttimkiem_TextChanged(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(keyword))
                ...
                dv.RowFilter = $"TenNCC LIKE '%{keyword}%' OR TenNV LIKE '%{keyword}%' OR MaPhieuTra LIKE '%{keyword}%'";

            gridControlphieudoitra.DataSource = dv;
        }
        ...
        private void TaoPopupLocTheo()
        {
            ...
            cboTT.Properties.Items.AddRange(new[] { "Tất cả", "Đã xác nhận", "Nháp", "Hủy" });
        ...
        private void btnApDung_Click(object sender, EventArgs e)
        {
            ...
            if (dateTu.EditValue != null)
            {
                ...
                filter += $"NgayTra >= #{fromDate:MM/dd/yyyy}# AND ";
            }
            if (dateDen.EditValue != null)
            {
                DateTime toDate = dateDen.DateTime.Date;
                filter += $"NgayTra <= #{toDate:MM/dd/yyyy}# AND ";
            }
            ...
            {
                filter += $"MaPhieuTra LIKE '%{txtSoPhieu.Text}%' AND ";
            }
            ...
            // Áp dụng bộ lọc
            gridViewphieutrahang.ActiveFilterString = filter;

        }
        private void btnMacDinh_Click(object sender, EventArgs e)
        {
            // Đặt lại tất cả điều kiện lọc về mặc định
            gridViewphieutrahang.ActiveFilterString = "";
        }
        private void btnthem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                string query = "SELECT TOP 1 MaPhieuTra FROM PhieuTraHang ORDER BY MaPhieuTra DESC";
                ...
                else
                {
                    ...
                    maMoi = "PT" + so.ToString("D3");
                }
                // Gọi form chi tiết và truyền mã mới
                chitietphieudoitra frm = new chitietphieudoitra(chitietphieudoitra.Mode.ThemMoi, maMoi);
                frm.ShowDialog();
                //
                LoadPhieuTraHangData();
            }
        ...
        private void btnsua_Click(object sender, EventArgs e)
        {
            ...
            int[] selectedRows = gridViewphieutrahang.GetSelectedRows();
            int rowHandle = -1;
            ...
            else
            {
                ...
                rowHandle = gridViewphieutrahang.FocusedRowHandle;
            }
            ...
            string maPhieu = gridViewphieutrahang.GetRowCellValue(rowHandle, "MaPhieuTra")?.ToString();
            ...
            // Mở form chi tiết
            chitietphieudoitra form = new chitietphieudoitra(chitietphieudoitra.Mode.ChinhSua, maPhieu);
            form.ShowDialog();

            // Reset lại biến khi reload
            userHasSelectedRow = false;
        }
        private void LoadPhieuTraHangData()
        {
            string connectionString = "Data Source=MINHTHUVU\\MINHTHU;Initial Catalog=QLBH_NhaThuoc;Integrated Security=True;Encrypt=False";
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                string query = "SELECT MaPhieuTra, TenNCC, TenNV, NgayTra, LyDoTra, TongTien, TrangThai FROM PhieuTraHang p JOIN NhaCungCap n ON p.MaNCC = n.MaNCC JOIN NhanVien nv ON p.MaNV = nv.MaNV";
        ...
        private void gridViewphieutrahang_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
        ...
        private void GridViewnphieutrahang_MouseUp(object sender, MouseEventArgs e)
        {
            var view = sender as DevExpress.XtraGrid.Views.Grid.GridView;
            if (view == null) return;
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            ...
            // Xác nhận xóa
            DialogResult result = MessageBox.Show($"Bạn có chắc chắn muốn xóa phiếu trả hàng [{maPhieu}] không?",
                                                  "Xác nhận xóa",
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    string connectionString = @"Data Source=MINHTHUVU\MINHTHU;Initial Catalog=QLBH_NhaThuoc;Integrated Security=True;Encrypt=False";
                    string query = "DELETE FROM PhieuTraHang WHERE MaPhieuTra = @MaPhieuTra";

                    using (SqlConnection conn = new SqlConnection(connectionString))
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@MaPhieuTra", maPhieu);
                        conn.Open();
                        cmd.ExecuteNonQuery();
                    }
